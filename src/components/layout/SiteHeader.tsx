    ...
    <header className="site-header">
      <Container className="header-inner">
        ...
        <LeadTrigger className="button button--primary button--small header-cta" intent="Get the latest price sheet">
          <span>Get price sheet</span><span aria-hidden="true">↗</span>
        </LeadTrigger>
        <button type="button" className="menu-toggle" aria-label="Toggle navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen((value) => !value)}>
          <span /><span />
        </button>
      </Container>
      <AnimatePresence>
        {menuOpen ? (
          <motion.nav className="mobile-nav" aria-label="Mobile navigation" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
